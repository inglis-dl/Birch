...
#ifndef __vtkBirchSliderWidget_h
#define __vtkBirchSliderWidget_h
...
class vtkBirchDoubleSlider;
//class ctkPopupWidget;
class vtkBirchSliderWidgetPrivate;

/// \ingroup Widgets
///
/// vtkBirchSliderWidget is a wrapper around a vtkBirchDoubleSlider and a QDoubleSpinBox
/// where the slider value and the spinbox value are synchronized.
/// \sa ctkRangeWidget, vtkBirchDoubleRangeSlider, QSpinBox
class vtkBirchSliderWidget : public QWidget
{
  Q_OBJECT
...
public:
  ...
  /// Constructors
  explicit vtkBirchSliderWidget(QWidget* parent = 0);
  virtual ~vtkBirchSliderWidget();
  ...
  /// vtkBirchSliderWidget forces the value to be within the
  ...
  /// False by default.
  /// Note: some sizing issues in the popup can happen if the vtkBirchSliderWidget
  /// has already parent. You might want to consider setting this property
  /// before setting a parent to vtkBirchSliderWidget.
  //bool hasPopupSlider()const;
  //void setPopupSlider(bool popup);

  ///
  /// Return the popup if vtkBirchSliderWidget hasPopupSlider() is true, 0 otherwise.
...
protected:
  QScopedPointer<vtkBirchSliderWidgetPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(vtkBirchSliderWidget);
  Q_DISABLE_COPY(vtkBirchSliderWidget);
